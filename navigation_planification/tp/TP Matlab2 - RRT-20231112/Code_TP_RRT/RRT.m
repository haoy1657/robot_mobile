...
% nseeds^2 + 2 must be less than or equal to the max trees since they are
% per axis
while nseeds^2+2>mt
    nseeds=nseeds-1;
...
if obstacles_avoid
    obs=obstacles_array2(use_objects_txt,nwalls,lim);
end
...
for i=3:(nseeds^2+2)
     rrt(i).valid=1;
     rrt(i).cords=seed_cords(i-2,:);
     rrt(i).parent=0;
     
     plot(rrt(i).cords(1),rrt(i).cords(2),'.g');
 end
...
initSearch(Iterations,lim,start,goal,treesMax,dodraw)
 ...
 for cur_it=1:Iterations
     ...
     [d2nodes]=nearestNeighbour(new_pnt);
 
     % CONNECT TO NEAREST NEIGHBOUR
     [objective]=connect2(d2nodes,new_pnt,start,goal,dodraw,cur_it,treesMax);
